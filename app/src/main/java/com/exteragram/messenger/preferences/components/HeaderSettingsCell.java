...
public class HeaderSettingsCell extends LinearLayout implements CustomPreferenceCell {
    ...
    public HeaderSettingsCell(Context context) {
        ...
        setOrientation(1);
        setGravity(17);
        Drawable drawableMutate = ContextCompat.getDrawable(context, R.drawable.ic_foreground_extera).mutate();
    ...
    public void setIsAyuGram(boolean z) {
        ...
        if (z) {
            ...
            Theme.ThemeInfo activeTheme = Theme.getActiveTheme();
            if (activeTheme.isMonet() && Build.VERSION.SDK_INT >= 31) {
                drawableMutate = ContextCompat.getDrawable(getContext(), R.drawable.ic_logo_foreground).mutate();
                drawableMutate.setColorFilter(new PorterDuffColorFilter(MonetUtils.getColor(activeTheme.isDark() ? "a1_200" : "a1_700"), PorterDuff.Mode.MULTIPLY));
                this.imageView.setBackgroundColor(MonetUtils.getColor(activeTheme.isDark() ? "a2_800" : "a1_100"));
            } else {
                drawableMutate = ContextCompat.getDrawable(getContext(), R.drawable.ic_logo_foreground2).mutate();
