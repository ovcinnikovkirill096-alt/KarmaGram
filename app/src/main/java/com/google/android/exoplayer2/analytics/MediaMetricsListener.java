...
public final class MediaMetricsListener implements AnalyticsListener, PlaybackSessionManager.Listener {
    ...
    public void onSessionActive(AnalyticsListener.EventTime eventTime, String str) {
        ...
        if (mediaPeriodId == null || !mediaPeriodId.isAd()) {
            ...
            this.metricsBuilder = MediaMetricsListener$$ExternalSyntheticApiModelOutline4.m().setPlayerName("ExoPlayerLib").setPlayerVersion("2.18.3");
    ...
    private void maybeUpdateMetricsBuilderValues(Player player, AnalyticsListener.Events events) {
        ...
        if (events.contains(2) && this.metricsBuilder != null && (drmInitData = getDrmInitData(player.getCurrentTracks().getGroups())) != null) {
            MediaMetricsListener$$ExternalSyntheticApiModelOutline5.m(Util.castNonNull(this.metricsBuilder)).setDrmType(getDrmType(drmInitData));
        }
    ...
    private void maybeReportPlaybackError(long j) {
        ...
        this.playbackSession.reportPlaybackErrorEvent(MediaMetricsListener$$ExternalSyntheticApiModelOutline1.m().setTimeSinceCreatedMillis(j - this.startTimeMs).setErrorCode(errorInfo.errorCode).setSubErrorCode(errorInfo.subErrorCode).setException(playbackException).build());
    ...
    private void maybeReportNetworkChange(long j) {
        ...
        if (networkType != this.currentNetworkType) {
            ...
            this.playbackSession.reportNetworkEvent(MediaMetricsListener$$ExternalSyntheticApiModelOutline3.m().setNetworkType(networkType).setTimeSinceCreatedMillis(j - this.startTimeMs).build());
        }
    }
    ...
    private void maybeReportPlaybackStateChange(Player player, AnalyticsListener.Events events, long j) {
        ...
        if (this.currentPlaybackState != iResolveNewPlaybackState) {
            ...
            this.reportedEventsForCurrentSession = true;
            this.playbackSession.reportPlaybackStateEvent(MediaMetricsListener$$ExternalSyntheticApiModelOutline0.m().setState(this.currentPlaybackState).setTimeSinceCreatedMillis(j - this.startTimeMs).build());
        }
    }
    ...
    private void reportTrackChangeEvent(int i, long j, Format format, int i2) {
        TrackChangeEvent.Builder timeSinceCreatedMillis = MediaMetricsListener$$ExternalSyntheticApiModelOutline2.m(i).setTimeSinceCreatedMillis(j - this.startTimeMs);
        if (format != null) {
