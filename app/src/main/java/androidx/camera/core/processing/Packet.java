...
public abstract class Packet {
    ...
    public static Packet of(Bitmap bitmap, Exif exif, Rect rect, int i, Matrix matrix, CameraCaptureResult cameraCaptureResult) {
        return new AutoValue_Packet(bitmap, exif, 42, new Size(bitmap.getWidth(), bitmap.getHeight()), rect, i, matrix, cameraCaptureResult);
    }

    public static Packet of(ImageProxy imageProxy, Exif exif, Rect rect, int i, Matrix matrix, CameraCaptureResult cameraCaptureResult) {
        return of(imageProxy, exif, new Size(imageProxy.getWidth(), imageProxy.getHeight()), rect, i, matrix, cameraCaptureResult);
    }

    public static Packet of(ImageProxy imageProxy, Exif exif, Size size, Rect rect, int i, Matrix matrix, CameraCaptureResult cameraCaptureResult) {
    ...
    public static Packet of(byte[] bArr, Exif exif, int i, Size size, Rect rect, int i2, Matrix matrix, CameraCaptureResult cameraCaptureResult) {
