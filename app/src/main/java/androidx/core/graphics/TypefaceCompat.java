...
public abstract class TypefaceCompat {
    ...
    private static Typeface getSystemFontFamilyWithFallback(FontResourcesParserCompat.ProviderResourceEntry providerResourceEntry) {
        ...
        List requests = providerResourceEntry.getRequests();
        if (requests.size() == 1) {
            return getSystemFontFamily(((FontRequest) requests.get(0)).getSystemFont());
        }
        if (Build.VERSION.SDK_INT < 31) {
            return null;
        }
        for (int i = 0; i < requests.size(); i++) {
            if (getSystemFontFamily(((FontRequest) requests.get(i)).getSystemFont()) == null) {
                return null;
            }
        }
        Typeface.CustomFallbackBuilder customFallbackBuilderM = null;
        for (int i2 = 0; i2 < requests.size(); i2++) {
            FontRequest fontRequest = (FontRequest) requests.get(i2);
            if (i2 == requests.size() - 1 && TextUtils.isEmpty(fontRequest.getVariationSettings())) {
                customFallbackBuilderM.setSystemFallback(fontRequest.getSystemFont());
                break;
            }
            ...
            if (TextUtils.isEmpty(fontRequest.getVariationSettings())) {
                try {
                    TypefaceCompat$$ExternalSyntheticApiModelOutline3.m();
                    TypefaceCompat$$ExternalSyntheticApiModelOutline4.m();
                    fontFamilyBuild = TypefaceCompat$$ExternalSyntheticApiModelOutline1.m(TypefaceCompat$$ExternalSyntheticApiModelOutline0.m(fontGuessPrimaryFont).setFontVariationSettings(fontRequest.getVariationSettings()).build()).build();
                } catch (IOException unused) {
                    Log.e("TypefaceCompat", "Failed to clone Font instance. Fall back to provider font.");
                    return null;
                }
            } else {
                fontFamilyBuild = TypefaceCompat$$ExternalSyntheticApiModelOutline1.m(fontGuessPrimaryFont).build();
            }
            if (customFallbackBuilderM == null) {
                customFallbackBuilderM = TypefaceCompat$$ExternalSyntheticApiModelOutline2.m(fontFamilyBuild);
            } else {
                customFallbackBuilderM.addCustomFallback(fontFamilyBuild);
            }
        }
        return customFallbackBuilderM.build();
    }
