...
public class BotFullscreenButtons extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        this.leftMenu.set(this.insets.left + AndroidUtilities.dp(8.0f), this.insets.top + AndroidUtilities.dp(8.0f), this.insets.left + AndroidUtilities.dp(38.0f) + AndroidUtilities.lerp(AndroidUtilities.lerp(this.closeText.getCurrentWidth(), this.backText.getCurrentWidth(), f4) + AndroidUtilities.dp(12.0f), fMin, f5), this.insets.top + AndroidUtilities.dp(38.0f));
        ...
        if (this.parentRenderNode != null && Build.VERSION.SDK_INT >= 31 && canvas.isHardwareAccelerated() && ((webView = this.webView) == null || webView.getLayerType() == 2)) {
            if (this.blurNode == null) {
                RenderNode renderNodeM = BotFullscreenButtons$$ExternalSyntheticApiModelOutline0.m("bot_fullscreen_blur");
                this.blurNode = renderNodeM;
                f = 16.0f;
                renderNodeM.setRenderEffect(RenderEffect.createBlurEffect(AndroidUtilities.dp(r3), AndroidUtilities.dp(18.0f), Shader.TileMode.CLAMP));
            } else {
                f = 16.0f;
            }
            RenderNode renderNodeM2 = BotFullscreenButtons$$ExternalSyntheticApiModelOutline1.m(this.parentRenderNode);
            this.blurNode.setPosition(0, 0, Math.max(1, renderNodeM2.getWidth() - AndroidUtilities.dp(f)), Math.max(1, (int) Math.min(this.insets.top + AndroidUtilities.dp(46.0f), renderNodeM2.getHeight())));
        ...
        float fLerp = AndroidUtilities.lerp(AndroidUtilities.dp(4.66f), AndroidUtilities.dp(5.5f), f4);
        float f7 = -fLerp;
        canvas.drawLine(AndroidUtilities.lerp(f7, 0.0f, f4), AndroidUtilities.lerp(f7, 0.0f, f4), fLerp, fLerp, this.iconStrokePaint);
        canvas.drawLine(AndroidUtilities.lerp(f7, 0.0f, f4), AndroidUtilities.lerp(fLerp, 0.0f, f4), fLerp, f7, this.iconStrokePaint);
        if (f4 > 0.0f) {
        ...
        canvas2.saveLayerAlpha(fDp6, f8, f9, f10, 255, 31);
        if (f5 <= 0.0f || this.previewText == null) {
            f2 = f5;
            f3 = 1.0f;
        } else {
            canvas2.save();
            canvas2.translate((this.leftMenu.left + AndroidUtilities.dp(30.0f)) - ((1.0f - f5) * fMin), this.leftMenu.centerY());
            f3 = 1.0f;
            this.previewText.ellipsize(((this.leftMenu.right - AndroidUtilities.dp(this.verifiedBackground != null ? 30.0f : 12.0f)) - (this.leftMenu.left + AndroidUtilities.dp(30.0f))) + 2.0f).draw(canvas2, 0.0f, 0.0f, -1, f5);
            canvas2 = canvas2;
            f2 = f5;
        ...
        if (f2 < f3) {
            ...
            float f11 = f3 - f4;
            if (f11 > 0.0f) {
                this.closeText.draw(canvas, (AndroidUtilities.dp(32.0f) * f2) + ((this.closeRect.left + AndroidUtilities.dp(30.0f)) - (AndroidUtilities.dp(r14) * f4)), this.closeRect.centerY(), -1, f11 * (f3 - f2));
            }
            if (f4 > 0.0f) {
                this.backText.draw(canvas, (AndroidUtilities.dp(32.0f) * f2) + this.closeRect.left + AndroidUtilities.dp(30.0f) + (AndroidUtilities.dp(r14) * f11), this.closeRect.centerY(), -1, f4 * (f3 - f2));
                canvas2 = canvas;
            } else {
                canvas2 = canvas;
            }
        ...
        float f12 = -fDp9;
        canvas2.drawLine(-fDp8, f12, 0.0f, fDp9, this.iconStrokePaint);
        canvas.drawLine(0.0f, fDp9, fDp8, f12, this.iconStrokePaint);
        canvas.restore();
        canvas.save();
        canvas.translate(this.menuRect.centerX() + AndroidUtilities.dp(f3), this.menuRect.centerY());
        ...
        canvas.drawCircle(0.0f, AndroidUtilities.dp(5.0f), AndroidUtilities.dp(1.66f), this.iconPaint);
        float f13 = this.animatedDownloading.set(this.downloading);
        if (f13 > 0.0f) {
            ...
            float f15 = 0.5f + fCurrentTimeMillis;
            canvas.save();
            canvas.clipRect(-AndroidUtilities.dp(5.0f), AndroidUtilities.lerp(-AndroidUtilities.dpf2(3.5f), AndroidUtilities.dpf2(3.5f), fCurrentTimeMillis), AndroidUtilities.dp(5.0f), AndroidUtilities.lerp(-AndroidUtilities.dpf2(3.5f), AndroidUtilities.dpf2(3.5f), f15));
            this.downloadPaint.setColor(Theme.multAlpha(-1, f3));
            canvas.drawPath(this.downloadPath, this.downloadPaint);
            canvas.restore();
            if (f15 > f3) {
                canvas.save();
                canvas.clipRect(-AndroidUtilities.dp(5.0f), AndroidUtilities.lerp(-AndroidUtilities.dpf2(3.5f), AndroidUtilities.dpf2(3.5f), 0.0f), AndroidUtilities.dp(5.0f), AndroidUtilities.lerp(-AndroidUtilities.dpf2(3.5f), AndroidUtilities.dpf2(3.5f), f15 - f3));
                this.downloadPaint.setColor(Theme.multAlpha(-1, f3));
                canvas.drawPath(this.downloadPath, this.downloadPaint);
                canvas.restore();
            }
