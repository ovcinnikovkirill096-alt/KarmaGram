...
public class BotDownloads {
    ...
    public static AlertDialog showAlert(Context context, String str, String str2, String str3, final Utilities.Callback callback, long j, String str4) {
        ...
        builder.setPositiveButton(LocaleController.getString(R.string.BotDownloadFileDownload), new AlertDialog.OnButtonClickListener() { // from class: org.telegram.ui.bots.BotDownloads$$ExternalSyntheticLambda2
            @Override // org.telegram.ui.ActionBar.AlertDialog.OnButtonClickListener
            public final void onClick(AlertDialog alertDialog, int i2) {
                BotDownloads.m19398$r8$lambda$0V5vVA5p02uOzaqNbeUDVEoiqw(zArr, callback, alertDialog, i2);
            }
        });
    ...
    public static /* synthetic */ void m19398$r8$lambda$0V5vVA5p02uOzaqNbeUDVEoiqw(boolean[] zArr, Utilities.Callback callback, AlertDialog alertDialog, int i) {
        if (zArr[0]) {
            return;
        }
    ...
    public static class DownloadBulletin extends Bulletin.ButtonLayout {
        ...
        private static class StatusDrawable extends Drawable {
            ...
            public void draw(Canvas canvas) {
                ...
                float f2 = this.animatedDone.set(this.done);
                if (f2 < 1.0f) {
                    float f3 = 1.0f - f2;
                    ...
                    this.doc.setAlpha((int) (f3 * 255.0f));
                    this.doc.draw(canvas2);
                    float fDp = AndroidUtilities.dp(14.0f);
                    this.strokePaint.setColor(Theme.multAlpha(-1, 0.2f * f3));
                    canvas2.drawCircle(f5, f6, fDp, this.strokePaint);
                    float f7 = f3 * 1.0f;
                    this.strokePaint.setColor(Theme.multAlpha(-1, f7));
                    this.rect.set(f5 - fDp, f6 - fDp, f5 + fDp, f6 + fDp);
                    float f8 = this.animatedHasPercent.set(this.hasPercent);
                    this.strokePaint.setColor(Theme.multAlpha(-1, f3 * 0.15f * (1.0f - f8)));
                    ...
                    this.strokePaint.setColor(Theme.multAlpha(-1, f7));
                    canvas2 = canvas;
                    canvas2.drawArc(this.rect, (-90.0f) - fMax, Math.max(0.02f, this.animatedProgress.set(this.progress)) * (-360.0f) * f8, false, this.strokePaint);
                    invalidateSelf();
                    canvas2.restore();
                } else {
                    f = 255.0f;
                ...
                if (f2 > 0.0f) {
                    ...
                    if (this.cancelled) {
                        canvas2.save();
                        canvas2.scale(f9, f9, iCenterX, iCenterY);
                    }
                    RLottieDrawable rLottieDrawable = this.doneDrawable;
                    if (rLottieDrawable != null) {
                        ...
                        this.doneDrawable.setAlpha((int) (f2 * f));
