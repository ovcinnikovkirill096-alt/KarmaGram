...
public class ShapeInput {
    ...
    /* JADX WARN: Code duplicated, block: B:49:0x0144 A[RETURN] */
    ...
    private boolean isInsideShape(float f, float f2) {
        ...
        if (this.shape.getType() == 1 || this.shape.getType() == 3) {
            ...
            if (f2 <= f10 || f2 >= f12) {
                if (f >= f7 || f <= f11) {
                    ...
                    double d3 = f - f11;
                    double d4 = f2 - f12;
                    fSqrt = (float) Math.sqrt(Math.min(Math.min(Math.pow(d, 2.0d) + Math.pow(d2, 2.0d), Math.pow(d2, 2.0d) + Math.pow(d3, 2.0d)), Math.min(Math.pow(d, 2.0d) + Math.pow(d4, 2.0d), Math.pow(d3, 2.0d) + Math.pow(d4, 2.0d))));
                } else if (f2 < f10) {
                    fSqrt = f10 - f2;
                } else if (f2 > f12) {
                    fSqrt = f2 - f12;
                ...
                if (fMin < AndroidUtilities.dp(f3)) {
                    return true;
                }
                return z;
            }
            ...
            if (this.shape.getType() == 3) {
                ...
                fMin = Math.min(fMin, distToLine(f, f2, shape5.centerX, shape5.centerY, shape5.middleX, shape5.middleY));
            }
            if (fMin < AndroidUtilities.dp(f3)) {
                return true;
            }
        ...
        if (this.shape.getType() == 4) {
            ...
            float fDistToLine = distToLine(f, f2, shape6.centerX, shape6.centerY, shape6.middleX, shape6.middleY);
            Shape shape7 = this.shape;
            if (Math.min(fDistToLine, distToLine(f, f2, shape7.radiusX, shape7.radiusY, shape7.middleX, shape7.middleY)) - (this.shape.thickness / 2.0f) < Math.min(size.width, size.height) * 0.1f) {
                return true;
            }
        }
        return false;
