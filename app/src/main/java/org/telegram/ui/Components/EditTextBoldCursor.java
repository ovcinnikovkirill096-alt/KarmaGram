    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.lineVisible && this.lineColor != 0) {
            ...
            if (this.lineActiveness < 1.0f) {
                canvas2.drawRect(getScrollX(), measuredHeight - iDp, getScrollX() + getMeasuredWidth(), measuredHeight, this.linePaint);
            }
            ...
            if (f3 > 0.0f) {
                ...
                canvas2.drawRect(getScrollX() + Math.max(0.0f, f4 - (this.activeLineWidth / 2.0f)), measuredHeight - ((int) (interpolation * AndroidUtilities.dp(2.0f))), getScrollX() + Math.min(f4 + (this.activeLineWidth / 2.0f), getMeasuredWidth()), measuredHeight, this.activeLinePaint);
            }
        }
        if (this.ellipsizeByGradient) {
            canvas2.save();
            ...
            this.ellipsizeGradient.setLocalMatrix(this.ellipsizeMatrix);
            canvas2.drawRect(getWidth() - getPaddingRight(), 0.0f, (getWidth() - getPaddingRight()) + this.ellipsizeWidth, getHeight(), this.ellipsizePaint);
            canvas2.restore();
            canvas2.restore();
        }
    }
