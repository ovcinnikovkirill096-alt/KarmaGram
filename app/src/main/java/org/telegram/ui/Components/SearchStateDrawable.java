...
public class SearchStateDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        if (i2 == 1) {
            ...
            f2 = i4 == 1 ? 1.0f - f7 : 0.0f;
        }
        if (i2 == 2) {
            f3 = this.fromState == 2 ? 1.0f : f7;
        } else {
            f3 = this.fromState == 2 ? 1.0f - f7 : 0.0f;
        }
        if (f > 0.0f) {
        ...
        if (f2 > 0.0f) {
            float fLerp = this.fromState == 2 ? AndroidUtilities.lerp(x(f5), x(f4), f2) : x(f4);
            canvas3.save();
            canvas3.rotate(f * 45.0f, this.cx, this.cy);
            drawLines(canvas3, (x(0.2452f) * f2) + fLerp, AndroidUtilities.lerp(y(0.5f), y(0.25f), f2), fLerp, y(0.5f), (x(0.2452f) * f2) + fLerp, AndroidUtilities.lerp(y(0.5f), y(f5), f2));
            canvas.restore();
        }
        if (f6 > 0.0f) {
            ...
            if (this.progressStart > 0) {
                ...
                if (getIconState() != 2 && !this.waitingForProgressToEnd) {
                    ...
                    fLerp2 = AndroidUtilities.lerp(fMin, Math.min(fLerp2, fMax + this.progressAngleTo), f6);
                }
                float f8 = this.progressAngleTo;
                float f9 = this.progressAngleFrom;
                boolean zContainsAngle = containsAngle(f8, f9 + fLerp2, f9 + fMin);
                ...
                this.progressRect.set(x(0.25f), y(0.25f), x(f5), y(f5));
