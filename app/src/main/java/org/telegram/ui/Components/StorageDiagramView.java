...
public abstract class StorageDiagramView extends View implements NotificationCenter.NotificationCenterDelegate {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (true) {
            ...
            ClearViewData clearViewData = clearViewDataArr[i3];
            if (clearViewData != null) {
                float f8 = this.drawingPercentage[i3];
                if (f8 != 0.0f) {
                    if (clearViewData.firstDraw) {
                        ...
                        if (Math.abs((float) (((3.141592653589793d * dWidth) / 180.0d) * ((double) f10))) <= 1.0f) {
                            ...
                            canvas.drawPoint(this.rectF.centerX() + ((float) (Math.cos(Math.toRadians(d)) * dWidth)), this.rectF.centerY() + ((float) (dWidth * Math.sin(Math.toRadians(d)))), this.data[i3].paint);
                        } else {
                            this.data[i3].paint.setStyle(Paint.Style.STROKE);
                            canvas.drawArc(this.rectF, (-90.0f) - (360.0f * f7), f10, false, this.data[i3].paint);
                        }
                    }
                    f7 += f8;
                }
        ...
        while (true) {
            ...
            ClearViewData clearViewData2 = clearViewDataArr2[i4];
            if (clearViewData2 != null) {
                float f12 = this.drawingPercentage[i4];
                if (f12 == 0.0f) {
                    i2 = i;
                } else {
                    if (clearViewData2.firstDraw) {
                        ...
                        if (Math.abs((float) (((double) f13) * ((dWidth2 * 3.141592653589793d) / 180.0d))) <= 1.0f) {
                            ...
                            canvas.drawPoint(this.rectF.centerX() + ((float) (dWidth2 * Math.cos(Math.toRadians(d2)))), this.rectF.centerY() + ((float) (Math.sin(Math.toRadians(d2)) * dWidth2)), this.data[i4].paint);
                            i2 = 255;
                        } else {
                            this.data[i4].paint.setStyle(Paint.Style.STROKE);
                            i2 = 255;
                            canvas.drawArc(this.rectF, (-90.0f) - (f11 * 360.0f), f13, false, this.data[i4].paint);
                        }
                    }
                    f11 += f12;
                }
