...
public abstract class BottomPagerTabs extends View {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (f > 0.0f) {
            ...
            float f6 = width;
            float f7 = f6 / 2.0f;
            float fFloor = (((float) Math.floor(this.progress)) * f6) + f7;
            float fCeil = (f6 * ((float) Math.ceil(this.progress))) + f7;
            float f8 = this.progress;
            float fLerp = paddingLeft + AndroidUtilities.lerp(fFloor, fCeil, f8 - ((int) f8));
            RectF rectF = AndroidUtilities.rectTmp;
            float f9 = iMin / 2.0f;
            rectF.set(fLerp - f9, AndroidUtilities.dp(9.0f), fLerp + f9, AndroidUtilities.dp(41.0f));
        ...
        while (true) {
            ...
            tab.clickRect.set(paddingLeft2, f2, paddingLeft2 + width, getHeight());
            ...
            float f12 = f4;
            float f13 = f5;
            rect.set((int) (tab.clickRect.centerX() - f11), AndroidUtilities.dp(f12), (int) (tab.clickRect.centerX() + f11), AndroidUtilities.dp(f10));
            float f14 = tab.nonscrollingT.set(fMin > 0.6f);
            if (f < 1.0f) {
                this.selectPaint.setColor(ColorUtils.setAlphaComponent(Theme.getColor(i2, this.resourcesProvider), (int) (f14 * 18.0f * (1.0f - f))));
                RectF rectF2 = AndroidUtilities.rectTmp;
                rectF2.set(rect);
                canvas.drawRoundRect(rectF2, AndroidUtilities.dp(f13), AndroidUtilities.dp(f13), this.selectPaint);
            }
