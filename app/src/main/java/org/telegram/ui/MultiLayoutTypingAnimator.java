...
public final class MultiLayoutTypingAnimator implements Choreographer.FrameCallback {
    ...
    public void setBlocks(ArrayList arrayList) {
        if (!this.blocks.isEmpty() && this.curBlockIdx >= this.blocks.size()) {
            ...
            this.curLineIdx = Math.max(0, ((MessageObject.TextLayoutBlock) this.blocks.get(size)).textLayout.getLineCount() - 1);
    ...
    /* JADX WARN: Code duplicated, block: B:25:0x005a A[PHI: r5
    ...
    private float computeRemainingPixels() {
        ...
        while (i < this.blocks.size()) {
            ...
            if (staticLayout != null) {
                int iMin = i == this.curBlockIdx ? Math.min(Math.max(this.curLineIdx, 0), Math.max(0, staticLayout.getLineCount() - 1)) : 0;
                for (int i2 = iMin; i2 < staticLayout.getLineCount(); i2++) {
                    float fLineWidth = lineWidth(staticLayout, i2);
                    if (fLineWidth > 0.001f) {
